			...
			if($this->form_validation->run() == TRUE){
				...
				if(in_array("all", $this->input->post('to_user'))){
					...
					foreach($this->input->post('to_user') as $key => $who){
						if($who == 'clients'){
							$clients = $this->ion_auth->users(array(3))->result();
						}elseif($who == 'admins'){
							$admins = $this->ion_auth->users(array(1))->result();
						}elseif($who == 'team_members'){
							$team_members = $this->ion_auth->users(array(2))->result();
						}else{
							$emails[] = $who;
						}
					}
				...
				$allEmails = array_merge(
					...
					extractEmails($clients),
					extractEmails($admins),
					extractEmails($team_members),
